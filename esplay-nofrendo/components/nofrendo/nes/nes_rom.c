...
#define  RESERVED_LENGTH   8
	...
	/* Read in the header */
//	FILE *fp = fopen(settings_load_str(SettingRomPath),"r");
//   _fread(&head, 1, sizeof(head), fp);
	printf("%s Head: %p (%x %x %x %x)\n",rominfo->filename , *rom, (*rom)[0], (*rom)[1], (*rom)[2], (*rom)[3]);
	memcpy(&head, *rom, sizeof(head));
	printf("rom_getheader memcpy");
	*rom += sizeof(head);
	printf("rom_getheader memcmp if");
	if (memcmp(head.ines_magic, ROM_INES_MAGIC, 4))
	{
		printf("rom_getheader:%s is not a valid ROM image", rominfo->filename);
	...
	} else
	{
		...
		printf("rom_getheader rom_adddirty");
...
rominfo_t *rom_load(const char *filename)
{
	...
	{
		printf("rom_load: load header error");
		goto _fail;
	}
	...
	{
		printf("rom_load:Mapper %d not yet implemented", rominfo->mapper_number);
	...
	*/
	...
	{
		printf("rom_load: alloc sram error");
		goto _fail;
	}
	...
	{
		printf("rom_load: load rom error");
		goto _fail;
	}
